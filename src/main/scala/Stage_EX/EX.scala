...
class EX extends Module {
  ...
  val io = IO(
    ...
    //  val btbTargetPredict    = Input(UInt(32.W))
      ...
      val wrongpredicted      = Output(Bool())
    }
  )
  ...
  ResolveBranch.PC            := io.PC
  ResolveBranch.immData       := io.immData
  ALU.ALUop                   := io.ALUop
...
ALU.src2 := Mux(io.op2Select === op2sel.rs2, alu_operand2, io.immData)
  ...
  io.exBranchTaken  :=  ResolveBranch.branchTaken
  io.exBranchAddr   := ResolveBranch.branchTarget

  // Prediciton Accuracy

  val actualTaken          = ResolveBranch.branchTaken
  val predictedTarget      = io.predictedTarget
  val predictedTaken       = io.predictedTaken
  val actualTarget         = ResolveBranch.branchTarget
  val usingPrediction      = io.predictorMode =/= 0.U
  val shouldUpdate         = usingPrediction && isBranchInstr

  io.updatePrediction   := shouldUpdate
  io.exUpdatePrediction := shouldUpdate

  val actualNextPC         = Mux(actualTaken, actualTarget, io.PC + 4.U)

when(!usingPrediction){
  io.wrongpredicted := true.B
}.elsewhen(usingPrediction && (predictedTarget =/= actualTarget)){
  io.wrongpredicted := true.B
}otherwise{
  io.wrongpredicted := false.B
}
